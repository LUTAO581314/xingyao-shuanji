...
for (const required of ["real engine and loopback model", "real shell outcomes", "HTTP evidence", "real engine upgrade", "real memory extraction engine", "startup decides the clean old host branch before migration", "product-integration", "compiled Windows product", "portable", "recovery", "workspace-draft-api.test.ts", "workspace-drafts.test.ts", "workspace-recovery.test.ts", "collaboration-api.test.ts", "delegation-api.test.ts"]) if (!clean.includes(required)) throw new Error(`缺少发行验收证据：${required}`)
...
const report: ReleaseValidationReport = { manifestHash: inspected.manifestHash, outcome: "passed", tests: {
  ...
  soulIntegration: evidence(["test/product-integration.test.ts", "test/tool-evidence-api.test.ts", "test/source-revisions.test.ts", "test/memory-review.test.ts", "test/memory-review-api.test.ts", "test/collaboration-api.test.ts", "test/delegation-api.test.ts", "test/compiled.test.ts", browserReportPath, memoryReportPath, collaborationReportPath, draftReportPath, raceReportPath, join(reports, `${stamp}-browser.txt`), join(reports, `${stamp}-memory-browser.txt`), join(reports, `${stamp}-collaboration-browser.txt`), join(reports, `${stamp}-draft-browser.txt`), join(reports, `${stamp}-draft-races.txt`)]),
