...
const maintenanceFiles = ["patches/opencode/001-filesystem-search-import-cycle.patch", "patches/opencode/002-portable-workspace-paths.patch", "patches/opencode/manifest.json", "patches/opencode/README.md", "docs/opencode-patches.md", "docs/system-design.md", "docs/implementation-status.md", "docs/workbench-modules.md"]
...
for (const name of ["xingyao.exe", "README.md", "THIRD-PARTY-NOTICES.txt", "CYTOSCAPE-LICENSE.txt", ...maintenanceFiles, ...(enginePath ? ["opencode.exe", ...(await Bun.file(join(output, "OPENCODE-LICENSE.txt")).exists() ? ["OPENCODE-LICENSE.txt"] : [])] : [])]) files[name] = new Bun.CryptoHasher("sha256").update(await Bun.file(join(output, name)).arrayBuffer()).digest("hex")
