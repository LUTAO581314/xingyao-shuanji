...
try {
  ...
  await expect(page.locator('#collaboration-count')).toContainText('3 个子会话 · 0 项星杳委派')
  ...
  await page.locator('#delegation-title').fill('核对协作闭环')
  await page.locator('#delegation-agent').selectOption('explore')
  await page.locator('#delegation-instruction').fill('检查创建、继续与显式交回的状态边界。')
  await page.locator('#delegation-create').click()
  await expect(page.locator('.collaboration-card')).toHaveCount(4)
  const owned = page.locator('.collaboration-card.delegation-owned').filter({ hasText: '核对协作闭环' })
  await expect(owned).toContainText('待核对或交回')
  await expect(owned).toContainText('已完成界面与状态边界检查')
  await owned.locator('[data-delegation-followup]').fill('继续核对是否复用同一子会话。')
  await owned.getByRole('button',{name:'继续',exact:true}).click()
  await waitFor(async () => { const value = await api(`/tasks/${task.id}/collaboration`); return value.delegations[0]?.state === 'waiting' && value.sessions.find(item => item.sessionID === value.delegations[0]?.sessionId)?.messageCount === 4 }, 10_000, 'continued owned delegation')
  await page.locator('#collaboration-refresh').click()
  await expect(owned).toContainText('继续操作复用了同一个子会话')
  expect(await page.locator('.collaboration-card.delegation-owned').count()).toBe(1)
  await owned.getByRole('button',{name:'交回主任务'}).click()
  await expect(owned).toContainText('已交回主任务')
  const mergedTask = await waitFor(async () => { const value = await api(`/tasks/${task.id}`); return value.status !== 'running' ? value : undefined }, 10_000, 'merged root response')
  expect(mergedTask.status).toBe('verifying')
  expect(mergedTask.messages.some(item => item.role === 'system' && item.text.includes('msg_child_assistant_2'))).toBe(true)
  expect(mergedTask.messages.some(item => item.role === 'assistant' && item.text.includes('可靠结论'))).toBe(true)
  ...
  await Bun.write(join(reports, `collaboration-browser-${version}.json`), JSON.stringify({ result: 'passed', version, executable, sha256, sessions: 4, delegations: 1, continuedSameSession: true, mergedToRoot: true, pageErrors, cspErrors, externalRequests }, null, 2))
  console.log(JSON.stringify({ result: 'passed', version, sessions: 4, delegations: 1, continuedSameSession: true, mergedToRoot: true, desktop: join(reports, `collaboration-browser-${version}-desktop.png`), mobile: join(reports, `collaboration-browser-${version}-mobile.png`) }))
} finally {
  await browser?.close()
