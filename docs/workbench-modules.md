...
选型与接口方案已明确。图谱后端在开发源码中投影已导入文档、记忆和真实经历；独立回归为 14 项通过、95 个断言。2026-09-17 随后运行完整 `bun test`：199 项通过、0 失败、8097 个断言、22 个文件，62.54 秒；`bun run typecheck` 通过。这是当前源码的回归结果，不是包含新图谱界面的发行验收。前端图谱入口、文件树与正文侧栏、子智能体面板、视觉创作适配仍需要接入和发行验收。当前已安装的 `0.1.0-dev.2` 尚没有这些新增入口。
