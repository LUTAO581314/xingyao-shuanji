...
最新本地候选为 `0.1.0-dev.8`：368 项完整测试与两组真实浏览器验收通过，增加有原话来源、归属与有效期的对话记忆审阅及有界后台整理。见 [对话记忆候选验收](verification-memory-review.md)。此前 `.7` 的引擎迁移依据保留在 [引擎迁移候选验收](verification-upgrade.md)；`.8` 尚未安装到现用身份。
