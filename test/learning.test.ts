...
describe("evidence-backed learning candidates", () => {
  ...
  test("uses Beta(1,1) and counts each tool call once despite multiple revisions", () => {
    ...
    expect(store.experience(first.id)?.retracted).toBe(true)
    expect(() => learning.propose(proposal([first.id, latest.id]))).toThrow("来源")
    const skill = learning.propose(proposal([latest.id, second.id, pending.id, latest.id]))
  ...
  test("new call revisions require fresh review and replace, rather than add, sample weight", () => {
    ...
    const latest = tool(store, task, "call-a", 1, "revised-result")
    ...
    expect(() => learning.promote(skill.id, active.revision, "旧来源不能复活")).toThrow("失效")
    const replacement = learning.propose(proposal([latest.id, ...skill.sourceIds.slice(1)]))
    const reviewed = learning.promote(replacement.id, replacement.revision, "已核查最新一次失败的适用条件")
    ...
    expect(learning.evaluate(replacement.id)).toMatchObject({ status: "active", independentSources: 3, unknownSources: 0 })
    store.recordAction({ id: "call-a", taskId: task.id, tool: "test-process", status: "unknown", text: "明确终止但结果无法判定", revision: "terminal-unknown",
      execution: { version: 1, lifecycle: "completed", outcome: "unknown", basis: "exit unavailable", exitCode: null, finishedAt: Date.now() } })
    expect(learning.evaluate(replacement.id)).toMatchObject({ status: "needs_review", independentSources: 2, unknownSources: 1 })
    expect(() => learning.promote(replacement.id, reviewed.revision)).toThrow("失效")
  })
