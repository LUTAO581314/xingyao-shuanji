...
test.skipIf(process.platform !== "win32" || !existsSync(executable))("compiled Windows product starts, serves real assets, checkpoints and restores on another host", async () => {
  ...
  try {
    ...
    expect((await api("/vendor/cytoscape.min.js")).headers.get("content-type")).toContain("javascript")
    expect(await (await api("/vendor/cytoscape.min.js")).text()).toContain("Cytoscape Consortium")
    expect((await (await api("/api/graph")).json()).nodes).toEqual([])
